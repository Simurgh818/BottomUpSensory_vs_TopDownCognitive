function [sub_ch_A_all, sub_ch_B_all, sub_pc_A_all, sub_pc_B_all, window_centers, k_opt, pc_labels] = ...
    ...
    % --- 1. CALCULATE RESTING STATE BACKGROUND (Start + End Concatenation) ---
    concat_bg_A = [];
    concat_bg_B = [];
    for w = 1:size(bg_windows, 1)
        idx_bg = find(time_ms_eeg >= bg_windows(w, 1) & time_ms_eeg <= bg_windows(w, 2));
        % Reshape to [Channels x (Time * Trials)]
        concat_bg_A = [concat_bg_A, reshape(trialsA_raw(:, idx_bg, :), num_ch, [])];
        concat_bg_B = [concat_bg_B, reshape(trialsB_raw(:, idx_bg, :), num_ch, [])];
    end
    ...
    for w = 1:num_windows
        ...
        % Calculate Correlation for the current active window across all trials
        act_A = reshape(trialsA_raw(:, w_s:w_e, :), num_ch, []);
        act_B = reshape(trialsB_raw(:, w_s:w_e, :), num_ch, []);
