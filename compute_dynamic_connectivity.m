function [sub_ch_A_all, sub_ch_B_all, sub_pc_A_all, sub_pc_B_all, window_centers, k_opt, pc_labels] = ...
    ...
    % --- 3. CALCULATE STATIC RESTING STATE BACKGROUND (0 - 0.5s ONLY) ---
    t_start_win = [0, 0.5];
    idx_bg = find(time_ms_eeg >= t_start_win(1) & time_ms_eeg <= t_start_win(2));
    
    % Isolate the raw baseline data
    bg_A_ch = avgA(:, idx_bg);
    bg_B_ch = avgB(:, idx_bg);
    
    % Baseline Correlation Matrices
    bg_ch_A = corrcoef(bg_A_ch');
    bg_ch_B = corrcoef(bg_B_ch');
    
    bg_pc_A = corrcoef((W * bg_A_ch)');
    bg_pc_B = corrcoef((W * bg_B_ch)');
